...
open class VideoPlayerView: UIView {
    ...
    public enum State {
        ...
        /// Video completed playing, iff isAutoPlay is false
        case completed
    }
...
@objc extension VideoPlayerView {
    ...
    open func play(for url: URL) {
        ...
        let videoURL = VideoManager.shared.reverseProxyURL(from: url)!
    ...
    open func seek(to time: CMTime, completion: ((Bool) -> Void)? = nil) {
        player?.seek(to: time, toleranceBefore: .zero, toleranceAfter: .zero) { completion?($0) }
    }
...
private extension VideoPlayerView {
    ...
    @objc func playerItemDidReachEnd(notification: Notification) {
        ...
        let prevState = state
        state = .completed
        stateDidChanged(state: state, previous: prevState)
